...
import { Heart, Calendar, Ruler, Venus, Mars, PawPrint } from "lucide-react"; // Importar iconos
import {
  Card,
  CardContent,
  CardFooter,
...
interface PetCardProps {
  pet: Pet & {
    ...
    size?: string | null;
    gender?: string | null;
    primary_image_url?: string | null; // Make sure this is included if not already
  };
...
function formatAge(years?: number | null, months?: number | null): string | null {
  ...
  if (years === 0 && months === 0) return null; // Return null if age is 0/unknown based on original logic
  let ageString = '';
  if (years > 0) ageString += `${years} ${years === 1 ? 'año' : 'años'}`;
  if (months > 0) {
    if (ageString) ageString += ' y ';
    ageString += `${months} ${months === 1 ? 'mes' : 'meses'}`;
  }
  return ageString || "Recién nacido"; // Fallback from original logic?
}
...
export default function PetCard({ pet, onFavorite, isFavorite = false }: PetCardProps) {
  ...
  const imageUrl = pet.primary_image_url; // Directly use the URL or null/undefined
  ...
  return (
    <Card className="group font-body overflow-hidden transition-all duration-300 hover:shadow-xl border border-border/40 h-full flex flex-col bg-card">
      <Link href={detailUrl} legacyBehavior={false} passHref className="flex-grow flex flex-col h-full">
          ...
          <div className="aspect-[4/3] relative w-full overflow-hidden">
            {imageUrl ? (
              ...
              <div className="w-full h-full flex items-center justify-center bg-neutral-200">
                <PawPrint className="w-12 h-12 text-neutral-400" />
              </div>
            )}
            <Button
              size="icon"
        ...
        <CardFooter className="px-4 py-3 border-t border-border/60 mt-auto">
          <Button 
            variant="ghost" 
            size="sm" 
            className="ml-auto text-xs font-body font-medium hover:text-primary h-auto py-1 px-2"
            tabIndex={-1}
            aria-hidden="true"
          >
            Ver detalles
